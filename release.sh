...
if [[ "$TYPE" =~ ^(patch|minor|major)$ ]]; then
    ...
    log_info "Releasing runuz v$VERSION -> v$NEW_VERSION ($TYPE bump)"
else
    NEW_VERSION="$TYPE"
    log_info "Releasing runuz v$NEW_VERSION (specific)"
fi
...
# Keep Cargo.lock's runuz entry in sync so the published crate and its
# lock agree (cargo would otherwise re-resolve on the next build).
cargo update -p runuz --precise "$NEW_VERSION" 2>/dev/null || true
...
if [ -z "$(git remote get-url origin 2>/dev/null)" ]; then
    git remote add origin git@github.com:adiled/runuz.git 2>/dev/null || true
fi
